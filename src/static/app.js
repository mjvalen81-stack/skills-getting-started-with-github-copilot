        ...
        if (details.participants && details.participants.length > 0) {
          ...
          details.participants.forEach((p) => {
            participantsHTML += `
              <li>
                <span>${p}</span>
                <button type="button" class="participant-delete-btn" data-activity="${name}" data-email="${p}" aria-label="Remove ${p}">✕</button>
              </li>`;
          });
          participantsHTML += "</ul>";
        } else {
          participantsHTML = "<p class='participants-none'><strong>Participants:</strong> None yet</p>";
        }
